 ...
 */
$.validator.addMethod('validateCity', function (value, el) {
	if ($.trim(value).length > 2) { return true; }
    return false;
}, Resources.VALIDATE_CITY);
...
$.validator.addMethod('ccnamevalidation', function (value, el) {
	
	var isOptional = this.optional(el);
    var isValid = (regex.alphabetOnly.test($.trim(value)))? true : false;
    return isOptional || isValid;
}, "Enter a valid name, Special characters & numbers not allowed");
