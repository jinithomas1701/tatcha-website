...
function contactUs() {
    ...
    app.getView('CustomerService', {pageId: 'contactus'}).render('content/contactus');
}
...
function submit() {
    ...
    var contactUsResult = contactUsForm.handleAction({
        send: function (formgroup) {
            ...
            var mailTo = Site.getCurrent().getCustomPreferenceValue('contactus-email');
            return Email.get('mail/contactus', mailTo)
                .setFrom(formgroup.email.value)
                .setSubject(formgroup.subject.value)
                .send({});
        },
        error: function () {
...
exports.Submit = guard.ensure(['post', 'https', 'csrf'], submit);
