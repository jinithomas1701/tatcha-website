...
function start() {
    ...
    if(empty(cart) || (empty(cart.object.allProductLineItems) && empty(cart.object.giftCertificateLineItems))) {
    ...
    if (customer.authenticated) {
        ...
        if (currentHttpParameterMap.emailExists != 'true'){
        	loginForm.clear();
        }
       ...
       var checkoutUrl = URLUtils.https('COCustomer-LoginForm').append('scope', 'checkout');
       var currentHttpParameterMap = request.httpParameterMap;
       ...
       if(Site.getCurrent().getCustomPreferenceValue('skip_email_gate')) {
	       ...
	       if (currentHttpParameterMap.emailExists == 'true'){
	    	   app.getView({
	               ContinueURL: checkoutUrl.append('emailExists', 'true'),
	               HasRefillProducts : HasRefillProducts 
	           }).render('checkout/checkoutlogin');
	       }else{
	    	   app.getView({
		            ContinueURL: checkoutUrl,
		            HasRefillProducts : HasRefillProducts 
	    	   }).render('checkout/checkoutlogin');
		   }
       }
...
function showLoginForm() {
    ...
    loginForm.handleAction({
        ...
        continuelogin:function () {
      	  ...
      	  if (customerByLogin) {
      	  response.redirect(URLUtils.https('COCustomer-Start').append('emailExists', 'true'));
      	  }
      	  else{
      		var cart = app.getModel('Cart').get();
      		Transaction.wrap(function () {
