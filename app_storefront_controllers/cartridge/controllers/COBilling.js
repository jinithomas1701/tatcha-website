...
function initAddressForm(cart) {
	try {
		var cartBillingAddress = cart.getBillingAddress();
		...
		var billingAddressFirstName = app.getForm('billing').object.billingAddress.addressFields.firstName.value;
		var billingAddressLastName = app.getForm('billing').object.billingAddress.addressFields.lastName.value;
		var shippingAddressFirstName = app.getForm('singleshipping').object.shippingAddress.addressFields.firstName.value;
		var shippingAddressLastName = app.getForm('singleshipping').object.shippingAddress.addressFields.lastName.value;
		
		// if firstname and lastname in billing address coming as NA, set shipping address first name and last name
		if(((billingAddressFirstName === 'NA' || billingAddressFirstName === null) && shippingAddressFirstName !== null) || 
			((billingAddressLastName === 'NA' || billingAddressLastName === null) && shippingAddressLastName !== null)) {
			
			app.getForm('billing').object.billingAddress.addressFields.firstName.value = ((billingAddressFirstName === 'NA' || billingAddressFirstName === null) && shippingAddressFirstName !== null) ? shippingAddressFirstName : 'NA';
			app.getForm('billing').object.billingAddress.addressFields.lastName.value = ((billingAddressLastName === 'NA' || billingAddressLastName === null) && shippingAddressLastName !== null) ? shippingAddressLastName : 'NA';
		}
...
function returnToForm(cart, params) {
    ...
    if(Site.getCurrent().getCustomPreferenceValue('enableSinglePageCheckout')) {
    	...
    	var hasBraintreeErrorparam = httpParameterMap.getParameterMap('braintreeError').parameterCount > 0 ? true : false ;
    	
    	if(hasBraintreeErrorparam) {
            app.getView({
                responseCode: 'INVALID-CREDITCARD',
                responseMsg: Resource.msg('braintree.creditcard.paymenterror', 'locale', null) 
	    ...
	    if (params) {
	        ...
	        if(!hasBraintreeErrorparam) {
	        	...
	        	app.getView(require('~/cartridge/scripts/object').extend(params, {
	                Basket: cart.object,
	                BraintreeError: session.custom.braintreeError,
	                ContinueURL: URLUtils.https('COBilling-Billing')
	            })).render('checkout/billing/billing');
	        }
	    } else {
...
function publicStart() {
    ...
    if (cart) {
    	...
    	//checkout login for AD - incase of force login from bag page when default address was not setting.
    	if(request.httpParameterMap.skip_email && request.httpParameterMap.skip_email.stringValue !== null){
        	var prefAddr = customer.addressBook ? customer.addressBook.preferredAddress : '';
        	if(!empty(prefAddr)){
	    		Transaction.wrap(function () {
	    			var address = cart.object.defaultShipment.createShippingAddress();
	    			address.setFirstName(prefAddr.firstName ? prefAddr.firstName : '');
	                address.setLastName(prefAddr.lastName ? prefAddr.lastName : '');
	                address.setAddress1(prefAddr.address1 ? prefAddr.address1 : '');
	                address.setAddress2(prefAddr.address2 ? prefAddr.address2 : '');
	                address.setCity(prefAddr.city ? prefAddr.city : '');
	                address.setPostalCode(prefAddr.postalCode ? prefAddr.postalCode : '');
	                address.setStateCode(prefAddr.stateCode ? prefAddr.stateCode : '');
	                address.setCountryCode(prefAddr.countryCode ? prefAddr.countryCode : '');
	                address.setPhone(prefAddr.phone ? prefAddr.phone : '');
	    		});
        	}
        }
    	
    	if (!cart.getCustomerEmail() && !customer.authenticated) {
...
function getGiftCertificateBalance() {
	...
	if(empty(request.getHttpReferer()) || request.getHttpReferer().indexOf(request.getHttpHost()) < 0) {
		responseUtils.renderJSON({
