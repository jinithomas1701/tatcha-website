...
function updateCartShippingMethod(cart) {
...
function start() {
    ...
    if (!Site.getCurrent().getCustomPreferenceValue('skip_email_gate') && (!cart.getCustomerEmail() && !customer.authenticated)) {
        app.getController('Cart').Show();
        return;
    }
	...
	if(hasAutoDeliveryProduct && !customer.authenticated && !customer.registered){
        response.redirect(URLUtils.https('Cart-Show').append('hasadproducts', 'true'));
	}
	
	if(!empty(cart) && cart.getDefaultShipment().getShippingMethod() == null){
		updateCartShippingMethod(cart);
	}
    ...
    if(customer.authenticated && customer.registered 
    	...
    	var values = {
		  ...
		  'countryCode' : app.getForm('singleshipping.shippingAddress.addressFields.country').value()
	    };
    ...
    if (cart.getProductLineItems().size() === 0) {
    	...
    	var isGiftOnlyShippingEdit = (request.httpParameterMap.shippingEdit && request.httpParameterMap.shippingEdit.stringValue == 'true') ? true : false;
    	
    	//setting gift certificate sender's email to cart
    	if((cart.object.giftCertificateLineItems && cart.object.giftCertificateLineItems.length > 0) && empty(cart.customerEmail) && cart.customerEmail == null ){
    		for(var i = 0; i < cart.object.giftCertificateLineItems.length; i++){
    			var giftItem = cart.object.giftCertificateLineItems[i];
    			if(!empty(giftItem) && !empty(giftItem.custom.giftCertificateSenderEmail) && giftItem.custom.giftCertificateSenderEmail !== null){
    				...
    				customerEmail = cart.getCustomerEmail() ? cart.getCustomerEmail() : customerEmail;
        ...
        if(Site.getCurrent().getCustomPreferenceValue('enableSinglePageCheckout')) {
        	...
        	if(isGiftOnlyShippingEdit) {
                ...
                }).render('singlepagecheckout/singlepagecheckout');
        ...
        pageMeta.update({
            pageTitle: Resource.msg('singleshipping.meta.pagetitle', 'checkout', 'Tatcha Checkout')
        });
        
        /* Single Page Checkout*/
        if(Site.getCurrent().getCustomPreferenceValue('enableSinglePageCheckout')) {
        	
        	var existingSessionState = (session.custom.checkoutState)?session.custom.checkoutState:'';
        	
        	// SET TO SHIPPING 
        	session.custom.checkoutState = 'shipping';      
        	session.custom.checkoutMode = 'edit';
        	
        	// Check if Billing is eligible
        	var showBilling = false;        	
        	if(request.httpParameterMap.checkoutState && request.httpParameterMap.checkoutState.stringValue !== null && request.httpParameterMap.checkoutState.stringValue == 'shipping'){
        		showBilling = false;
        	} else  if(existingSessionState && (existingSessionState == 'billing' || existingSessionState == 'review') && !session.customer.authenticated) {        		
        		// Extra validation to check if Billing eligible     
        		if(require('int_singlepagecheckout/cartridge/scripts/util/SinglePageCheckoutUtils').checkIfShippingInfoSet(cart)){
        			showBilling = true;
        		}
        	}
    ...
    singleShippingForm.handleAction({
        save: function () {
            ...
            if(Site.getCurrent().getCustomPreferenceValue('skip_email_gate')) {
            	if(!customer.authenticated && !handleCustomerLogin()) {
            		return;
            	}
            }
            ...
            if(request.httpParameterMap.dwfrm_singleshipping_addressList) {
            	session.custom.selectedShippingAddress = request.httpParameterMap.dwfrm_singleshipping_addressList.value;
            }
            
            var directToSummaryPage = request.httpParameterMap.paymentInfoAvailable;
            if(directToSummaryPage.booleanValue) {
...
function handleCustomerLogin() {
	...
	var CustomerMgr = require('dw/customer/CustomerMgr');
	var customerEmail = request.httpParameterMap.customeremail;
...
function selectShippingMethod() {
    ...
    //Assign a shipping method if a user switch shipping metod during express checkout
    session.custom.expresshippingmethod = request.httpParameterMap.shippingMethodID.stringValue;
    session.custom.expresshippingaddressid = request.httpParameterMap.id.stringValue;
    ...
    var shippingOnlyEdit=false;
    if(request.httpParameterMap.isShippingEdit.booleanValue){
    	shippingOnlyEdit=true;
    }

    /*
     * Single Page Checkout
     */
    if(Site.getCurrent().getCustomPreferenceValue('enableSinglePageCheckout') && !shippingOnlyEdit) {
	    app.getView().render('singlepagecheckout/ordersummary')
    }else if(Site.getCurrent().getCustomPreferenceValue('enableSinglePageCheckout') && shippingOnlyEdit) {
	    app.getView({
	    	shippingOnlyEdit: true
	    }).render('singlepagecheckout/ordersummary')
    } 
    else {
        app.getView({
            Basket: cart.object
...
function estimateShippingMethod(){
	Transaction.wrap(function () {
...
//	address = new TransientAddress();
	...
	var postalCode = request.httpParameterMap.postalCode.stringValue;
...
function updateShippingMethodList() {
    ...
    if(Site.getCurrent().getCustomPreferenceValue('enableSinglePageCheckout')) {
        ...
        }).render('singlepagecheckout/shippingmethods');    	
    } else {
        app.getView({
            Basket: cart.object,
            ApplicableShippingMethods: applicableShippingMethods,
...
function getApplicableShippingMethodsJSON() {
    ...
    if(address.countryCode && address.postalCode) {
    	session.custom.fedPostalCode = address.postalCode;
		session.custom.fedCountryCode = address.countryCode;
    }
...
function updateCustomerEmail() {
	var cart = app.getModel('Cart').get();
	var userEmail = request.httpParameterMap.email.stringValue;
	
	if(userEmail) {
		Transaction.wrap(function () {
			cart.setCustomerEmail(userEmail);
		});
	}
...
exports.EstimateShippingMethod = guard.ensure(['post'], estimateShippingMethod);
exports.UpdateCustomerEmail = guard.ensure(['post', 'csrf'], updateCustomerEmail);
