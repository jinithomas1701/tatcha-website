 ...
 * renders an error page (util/redirecterror template).
 */
function start() {
    var redirect = URLRedirectMgr.getRedirect(),
        location = redirect ? redirect.getLocation() : null;
        
    //ApplePay Verification Redirect
    var Site = require('dw/system/Site');
    var applePayUrl = Site.getCurrent().getCustomPreferenceValue('ApplePayRequestUrl');
    if(!empty(request.httpHeaders["x-is-path_info"]) && 
		request.httpHeaders['x-is-path_info'] == applePayUrl) {
    	app.getView().render('util/applepayverification');
    	return;
    }
