...
//Avalara change: Call site for site preferences
...
var CartModel = AbstractModel.extend({
    ...
    calculate: function () {
    	...
    	if (avaconfig.taxCalculation && !session.custom.NoCall) 
    	{ 
    		//require('int_avatax/cartridge/scripts/app').getController('Avatax').CalculateTaxes(this.object);
			require('int_avatax_sfra/cartridge/scripts/hooks/avatax/avataxhooks').calculateTax(this.object);
    		
    	}
    	else if (!avaconfig.taxCalculation && !session.custom.NoCall)
    		{
    		calculateTax(this.object);
    		}
    },
    ...
    addProductToCart: function() {
        ...
        // Adds a product.
            ...
            if (productToAdd.object.isProductSet()) {
                ...
                if (params.Quantity.doubleValue > 0) {
     ...
     /**
     * Check if the product has reached maxQty.
     */
     isMaxQtyReached: function () {
        var cart = this;
        var status = 0;
        var params = request.httpParameterMap;
        if(!empty(params.pid.stringValue) && !empty(params.page.stringValue)){
                var page = params.page.stringValue;
                var Product = app.getModel('Product');
                var productToAdd = Product.get(params.pid.stringValue);         
                ...
                var reqQty = (productInCart && !productInCart.optionModel) ? (productInCart.quantityValue + parseInt(params.Quantity.doubleValue)) : parseInt(params.Quantity.doubleValue); 
                if(reqQty > maxQty) {
                	status = maxQty;
                 }                  
         }
    ...
    addProductItem: function (product, quantity, productOptionModel) {
        ...
        Transaction.wrap(function () {
            ...
            if (product) {
                ...
                var reqQty = (productInCart && !productInCart.optionModel) ? (productInCart.getQuantity().value + quantity) : quantity; 
                if(reqQty > maxQty) {
                	session.custom.maxQtyError = dw.web.Resource.msgf('cart.maxqtyerror', 'checkout', null, defaultMaxQty);
                	if(product){
                		session.custom.maxQtyError_pid = product.ID;
                	}
            		return false;
                }
    ...
    preCalculateShipping: function (shippingMethod) {
        ...
        if (shipment) {
            ...
            var shippingTotalPrice = this.getShippingTotalPrice();
            var adjustedShippingTotal = this.getAdjustedShippingTotalPrice();
            ...
            return {
                shippingExclDiscounts: shippingTotalPrice,
                shippingInclDiscounts: adjustedShippingTotal,
                productShippingCosts: productShippingCosts,
                productShippingDiscounts: productShippingDiscounts,
                shippingPriceAdjustments: priceAdjArray,
                shippingPriceAdjustmentsTotal: priceAdjTotal,
    ...
    updateShipmentShippingMethod: function (shipmentID, shippingMethodID, shippingMethod, shippingMethods) {
        ...
        var shippingCountry = !empty(shipment.shippingAddress) ? shipment.shippingAddress.countryCode : null;
        if(!empty(shippingMethodID) && shippingCountry != 'US') {
         return;
        }
     ...
     */
    //Avalara changes: orderNo as a parameter and include orderNo in the OrderMgr.createOrder call
    ...
    updateGiftWrapOptions: function() {
    	var giftWrapId = Site.getCurrent().getCustomPreferenceValue('GiftWrapId');
		var cart = this;
		var hasGiftWrap = this.checkCartHasGiftWrap();
		...
		for (var i = 0, il = plis.length; i < il; i++) {
	       var item = plis[i];
	       if (!empty(item.product) && (item.product.ID == giftWrapId) && (plis.length == 1 || giftWrapEligibility == 'ineligible')) {
	    	   Transaction.wrap(function () {
	               cart.removeProductLineItem(item);
	               cart.object.defaultShipment.gift = false;
	               //cart.object.defaultShipment.giftMessage = '';
	           });
	       }
		}
   ...
   getTotalCartQuantity: function () {
	   ...
	   while (pliIt.hasNext()) {
		...
		cartQty += cart.giftCertificateLineItems ? cart.giftCertificateLineItems.size() : 0;
		return cartQty;
   },
   
   addMultipleItems: function(list) {
	   var cart = this;
	   var Product = app.getModel('Product');
       var productOptionModel;
       var previousBonusDiscountLineItems = cart.getBonusDiscountLineItems();
       ...
       for(var i=0; i < list.length; i++) {
       		var id = list[i]['id'];
       		var qty = parseInt(list[i]['qty']);
       		var options = list[i]['options'] ? list[i]['options'] : request.httpParameterMap;
       		var productToAdd = Product.get(id);
	       	if(productToAdd) {
	       		productOptionModel = productToAdd.updateOptionSelection(options);
   ...
   checkGiftItem: function() {
	   var cart = this;
	   var giftAdded = false;
	   var giftItemQuantity = 0;
	   var giftLineItem = null;
	   var Product = app.getModel('Product');
	   var productOptionModel;
       ...
       var productLineItems = cart.object.productLineItems;
       for (var q = 0; q < productLineItems.length; q++) {
	       	if (productLineItems[q].productID === giftBuilderSKU && productLineItems[q].custom.giftBuilderSku != 'GiftSetMaster') {
	       		giftAdded = true;
	       		giftItemQuantity = productLineItems[q].quantityValue;
	       		giftLineItem = productLineItems[q];
	       		break;
	       	}
       }
       
       if(giftAdded == true){
    	   var mainGiftProduct = Product.get(giftBuilderSKU);
    	   var ProductUtils = require('app_storefront_core/cartridge/scripts/product/ProductUtils.js');
    	   var items = ProductUtils.giftBuilderProductList(mainGiftProduct,false);
    	   var offerProducts = items.offerProducts;
    	   for (var q = 0; q < productLineItems.length; q++) {
    		   for (var r = 0; r < offerProducts.length; r++) {
   	       			if (productLineItems[q].productID == offerProducts[r].ID && productLineItems[q].custom.giftBuilderSku == '') {
   	       				productQuantity = productQuantity+productLineItems[q].quantityValue;
   	       			}
    		   }
    	   }
    	   var newQuantity = parseInt(productQuantity/3);
    	   if(newQuantity < giftItemQuantity){
    		   productOptionModel = mainGiftProduct.updateOptionSelection(request.httpParameterMap);
    		   if(newQuantity == 0){
    			   Transaction.wrap(function () {
    				   cart.removeProductLineItem(giftLineItem);
                       cart.calculate();
                   });
    		   }else{
    			   Transaction.wrap(function () {
                       cart.updateLineItem(giftLineItem, mainGiftProduct.object, newQuantity, productOptionModel);
                       cart.calculate();
                   });
    		   }
   ...
   checkMaximumQty: function(productID) {
		var status = false;
		var cart = this;
		if(productID){
			...
			var productInCart;
			for (var q = 0; q < productLineItems.length; q++) {
			        if (productLineItems[q].productID === product.object.ID) {
			                productInCart = productLineItems[q];
			                break;
			        }
			}
			if(!empty(productInCart)){
				var defaultMaxQty = Site.getCurrent().getCustomPreferenceValue('maxOrderQuantity');
				var maxQty = !empty(product.object.custom.maxOrderQuantity) ? product.object.custom.maxOrderQuantity : defaultMaxQty;
				if(productInCart.quantityValue >= maxQty) {
					status = true;
				}          
			}
		}
  ...
  updateCoupons: function() {
	  var cart = this;
	  cart.calculate();
	  for(var i = 0; i < cart.object.couponLineItems.size(); i++) {
  		var coupon = cart.object.couponLineItems[i];
