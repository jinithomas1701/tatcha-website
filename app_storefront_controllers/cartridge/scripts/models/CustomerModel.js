...
CustomerModel.login = function (username, password, rememberMe) {
    ...
    authenticatedCustomer = Transaction.wrap(function () {
        ...
        return CustomerMgr.loginCustomer(authStatus, empty(rememberMe)?false:rememberMe);
    });
...
CustomerModel.createAccount = function (email, password, form) {
    ...
    if(!empty(phoneMobile)) {        		
		if(session.custom.phoneVerified){
			newCustomer.object.profile.custom.isVerifiedPhone = true;
    ...
    return Transaction.wrap(function () {
    	var authStatus = CustomerMgr.authenticateCustomer(email, password);
        return CustomerMgr.loginCustomer(authStatus, empty(rememberMe)?false:rememberMe);
    });
...
/**
 * Updates a customer profile with form data.
 * @transactional
 * @alias module:models/CustomerModel~CustomerModel/editProfile
 * @param form the form to invalidate
 * @return <code>true</code> if transaction is successfully committed, <code>false</code> if
 * <ul><li>the customer profile values cannot be set</li></ul>
 */
CustomerModel.editProfile = function (form) {
 ...
 * <ul><li>the customer profile values cannot be set</li></ul>
 */
CustomerModel.editEmail = function (login, form) {

    Transaction.begin();
    
  //@FIXME Put on customer scope vs. passing in customer
    if ((customer === null) || (login === null) || (customer.profile === null)) {
        return false;
    }
    var result = customer.profile.credentials.setLogin(login);
 ...
 * <ul><li>the customer profile values cannot be set</li></ul>
 */
CustomerModel.editPassword = function (password, currentPassword) {
...
/**
 * Updates a customer account with phone number.
 * @transactional
 */
CustomerModel.updateCustomerPhone = function (phoneMobile,countryCode,verified, deletePhone) {

	if((empty(phoneMobile) || empty(countryCode)) && !deletePhone){
		return false;
	}
    ...
    customer.profile.phoneMobile = phoneMobile;
    customer.profile.custom.isVerifiedPhone = verified;  
...
CustomerModel.updateCustomerPhoneByCustomerData = function(customerData, phoneMobile,countryCode,verified) {
	if((empty(customerData) || empty(phoneMobile) || empty(countryCode))){ 
		return false;
	}
	
	Transaction.begin();
    
    if ((customerData === null) || (customerData.object.profile === null)) {
     return false;
    }
    
    customerData.object.profile.phoneMobile = phoneMobile;
    customerData.object.profile.custom.isVerifiedPhone = verified;  
    customerData.object.profile.custom.countryCode = parseInt(countryCode);
