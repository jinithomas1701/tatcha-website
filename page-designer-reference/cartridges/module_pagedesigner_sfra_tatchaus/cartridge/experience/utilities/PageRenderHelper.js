...
module.exports = {
    ...
    getCanonicalUrl: function (page){
        var canonicalUrl;
        var canonicalContent = dw.content.ContentMgr.getContent('canonical-url');
        if(canonicalContent){
            var canonicalJSON = JSON.parse(canonicalContent.custom.body);
            var pageID = page.ID;
            if(canonicalJSON && canonicalJSON.hasOwnProperty(pageID)){
                canonicalUrl = canonicalJSON[pageID];
            }
        }
        return canonicalUrl;
    }
