...
module.exports.render = function(context) {
	...
	if (content.enableOverlay) {
		model.enableOverlay = content.enableOverlay;
	}
	...
	if (content.mobileBlockPositionVertical) {
		model.mobileBlockPositionVertical = content.mobileBlockPositionVertical;
	}
	
	if (content.mobileTextAlignment) {
		model.mobileTextAlignment = content.mobileTextAlignment;
	}
	...
	if(request && request.httpParameterMap && !empty(request.httpParameterMap.params) && request.httpParameterMap.params){
		var reqParamsValue = JSON.parse(request.httpParameterMap.params.value);
		if(reqParamsValue && reqParamsValue.custom){
