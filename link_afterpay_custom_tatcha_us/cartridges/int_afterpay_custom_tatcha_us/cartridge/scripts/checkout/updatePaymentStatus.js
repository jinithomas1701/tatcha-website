 ...
 */
base.handlePaymentStatus = function (order){
    var response;
    ...
    if (response === 'SERVICE_UNAVAILABLE' || response.httpStatusCode === 500 || response === 'INTERNAL_SERVICE_ERROR') {
        finalPaymentStatus = require('int_afterpay_core/cartridge/scripts/checkout/afterpayIdempotency').delayPayment(order, apInitialStatus);
    }
