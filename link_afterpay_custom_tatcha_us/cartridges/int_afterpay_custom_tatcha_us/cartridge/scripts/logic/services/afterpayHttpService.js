...
var AfterpayHttpService = Class.extend({
    ...
    call: function (urlPath, httpVerb, requestBody, errorDelegate, action) {
        var serviceArgs;
        var service;
        var result;
        ...
        service = LocalServiceRegistry.createService(serviceID, {
            createRequest: function (reqService, args) {
                if(httpVerb==='UPDATE_SHIPPING'){
                    reqService.URL=reqService.URL.replace("/v1","/v2");
                }

                if(httpVerb==='CREATE_ORDER' && action ==='createToken'){
                    reqService.URL=reqService.URL.replace("/v1","/v2");
                    reqService.setURL(reqService.URL + args.endPointUrl);
                    reqService.setRequestMethod(args.method);
                } else if(httpVerb==='GET_ORDER' && action ==='getOrder') {
                    var token = requestBody.token;
                    reqService.URL=reqService.URL.replace("/v1","/v2");
                    reqService.URL = reqService.URL  +  args.endPointUrl  + '/' +token;
                    reqService.setURL(reqService.URL);
                    reqService.setRequestMethod('GET');
                } else {
                    reqService.setURL(reqService.URL + args.endPointUrl);
                    reqService.setRequestMethod(args.method);
                }
    ...
    getServiceID: function (serviceIdentifier) {
        switch (serviceIdentifier) {
            ...
            case 'UPDATE_SHIPPING' : return this.afterpaySitePreferencesUtilities.getServiceName();
            case 'GET_ORDER' : return this.afterpaySitePreferencesUtilities.getServiceName();
