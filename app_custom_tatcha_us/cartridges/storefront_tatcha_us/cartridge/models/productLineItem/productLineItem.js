...
module.exports = function productLineItem(product, apiProduct, options) {
    ...
    productDecorators.images(product, apiProduct, { types: ['large'], quantity: 'single' });
    ...
    productDecorators.wishlist(product, apiProduct.ID);
    productDecorators.customFields(product, apiProduct, 'productLineItem', options);
    productDecorators.analyticFields(product, apiProduct);
    ...
    productLineItemDecorators.gwpProduct(product, options.lineItem);
    productLineItemDecorators.proratedPrice(product, options.lineItem);
    return product;
};
