...
function getPopularSearchPhrases(suggestModel) {
	var suggestions = suggestModel.getPopularSearchPhrases();
	if (!suggestions) {
		return {
			available: false
		};
	...
	return {
		available: suggestions.hasNext(),
		popularPhrases: suggestions
	};
}
...
module.exports = function (searchPhrase, maxSuggestions) {
	...
	var popularSearchPhrases = getPopularSearchPhrases(suggestModel);
	...
	return {
		...
		content: content,
		custom: custom,
		popularSearchPhrases: popularSearchPhrases
	};
};