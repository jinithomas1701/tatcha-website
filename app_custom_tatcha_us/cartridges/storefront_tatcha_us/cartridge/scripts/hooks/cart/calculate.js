...
exports.calculate = function (basket) {
    ...
    //Avalara change: and add the following conditional statement to avoid calling the default calculateTax if Avatax is enable
    if(!avaconfig.taxCalculation || session.custom.NoCall
        || (basket.defaultShipment && basket.defaultShipment.shippingAddress && basket.defaultShipment.shippingAddress.countryCode.value != 'US')) {
        HookMgr.callHook('dw.order.custom.calculateTax', 'calculateTax', basket);
    }
...
function calculateProductPrices (basket) {
    ...
    while (productLineItems.hasNext()) {
        ...
        } else if((product.ID !== dw.system.Site.getCurrent().getCustomPreferenceValue('GiftWrapId')) &&
            (!product.availabilityModel || !product.availabilityModel.inStock)){
            productLineItem.setPriceValue(0);
        } else {
            //change code logic to set up promotion price for auto-delivery item
            if(productLineItem.custom.sordeliveryoption){
                productLineItem.setPriceValue(productPrices.get(product).valueOrNull - (productPrices.get(product).valueOrNull/10) );
            } else {
