...
function CompareList() {
    ...
    */
    ...
    /**
     * Gets the list of applicable attribute groups for a given list of category Ids.
     */
    this.getAttributeGroupsForCategories = function (categories) {
    	var categoryKeys = categories.keySet();
    	var categoryKeysIterator = categoryKeys.iterator();
    	var attributeGroups;
        while (categoryKeysIterator.hasNext()) {
        	var category = dw.catalog.CatalogMgr.getCategory(categoryKeysIterator.next());
        	attributeGroups = findAttributeGroups(category);
    ...
    /**
     * Gets the list of applicable attribute groups for given a category Id.
     */
    this.getAttributeGroupsForCategory = function (categoryId) {
    	var category = dw.catalog.CatalogMgr.getCategory(categoryId);
    	attributeGroups = findAttributeGroups(category);
    	return attributeGroups;
    }
     ...
     *
     */
    this.findComparisonAttributeGroups = function () {
        // Gets the product attribute model.
        var model = new dw.catalog.ProductAttributeModel();
        ...
        while (persistentAttributeGroups.hasNext()) {
            ...
            /*
             * -----	IMPORTANT!	------
             * Added customization to the comparison flow to take attributes only from the category from where it is called.
             * The attribute group for the main category should be defined with the ID comparison. The attribute should 
             * also be visible for this to display.
             */
            if('comparison' == persistentAttributeGroup.ID) {
	            var persistentAttributeDescriptors = model.getVisibleAttributeDefinitions(persistentAttributeGroup).iterator();
