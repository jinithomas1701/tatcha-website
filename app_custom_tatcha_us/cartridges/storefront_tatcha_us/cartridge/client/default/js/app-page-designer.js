...
function initializeEvents() {
	...
	$('.vsa-action-btn').on('keypress',function(e) {
		if(e.keyCode === 13){
			$(this).trigger("click");
		}
    });

	$('.home-page-marquee-btn').on('keypress',function(e) {
		if(e.keyCode === 13){
			$(this).trigger("click");
		}
    });
...
var SOR = {
	...
	removeRefill: function(row) {console.log(row.find('.osf-remove-link'));
		progress.showFull();
	...
	init: function() {
	    ...
	    $('.account-switch-section').on('click', function(){
	    	...
	    	if(name) {
	    		...
	    		if(name == 'summary') {
	    			$(this).parents('.panel').find('form')[0].reset();
	    			$(this).parents('.panel').find('form .has-error').removeClass('has-error');
	    			$(this).parents('.panel').find('.alert.alert-danger').remove();
	    			$(this).parents('.panel').find('form span.help-block').html('');
	    			if($(this).parents('.panel').find('form').attr('id') == 'ChangePassowrdForm') {
	    				$(this).parents('.panel').find('form input').val('');
	    			}
	    			if($(this).parents('.panel').find('form').attr('id') == 'EmailForm') {
	    				var defVal = $(this).parents('.panel').find('form #dwfrm_profile_customer_email').attr('data-default-value');
	    				$(this).parents('.panel').find('form #dwfrm_profile_customer_email').val(defVal);
	    				$(this).parents('.panel').find('form #dwfrm_profile_customer_emailconfirm').val('');
	    			}
	    		}
	    	}
	    });
...
// $(document).on("keyup", '.nav-shop-all.nav-desktop, .nav-gifts.nav-desktop',function(event){
...
//    }
 ...
 $('.dismiss-homescreen-banner').click( function() {
   $('.homescreen-banner').hide();
    ...
    $('#subscribeFotter').on('hidden.bs.modal', function () {
        var urlWithoutParam = location.href.split('?')[0];
    ...
    $(document).on('blur', '#braintreeCardOwner', function(event){
         var braintreeCreditCardOwner=$(this).val().trim();
    	 if ((braintreeCreditCardOwner=='')||(!braintreeCreditCardOwner.match(/^[A-Za-z ]+$/))) {
  		 $('.braintreeCardOwnerError').text(Resources.VALIDATE_CREDITCARDOWNERNAME);
  		 $('.braintree-text-input').addClass('has-error');
  		 $('.braintreeCardOwnerError').css('display','block');
  	  }
  	  else{
  		$('.braintreeCardOwnerError').text("");
  		$('.braintree-text-input').removeClass('has-error');

  	  }
  });

    $('.credit-card-save').click(function() {
    ...
    $( ".nav-desktop li.dropdown.has-subnav .nav-link,  .nav-desktop li.navbar-item .nav-link, .navbar-brand, .search-form-list input" ).focus(function(event) {
    	var parent = $(this).parent();

		if ($(this).hasClass('nav-link')) {
			//$(parent).addClass("highlightNav");
		}
...
$(document).on("keyup", '.nav-desktop .dropdown-menu a', function(e){
	if(e.keyCode === 32){
        e.preventDefault();
		$(this)[0].click();
	}
...
$(document).on("click",".video-play-pause.play",function(){
		$(this).parent().find('.player-desktop-hero')[0].play()
        $(this).removeClass("play");
        $(this).addClass("pause");
        $(this).attr('aria-label', 'Pause homepage video');
   })
   $(document).on("click",".video-play-pause.pause",function(){
		$(this).parent().find('.player-desktop-hero')[0].pause()
       $(this).removeClass("pause");
       $(this).addClass("play");
       $(this).attr('aria-label', 'Play homepage video');
   })

   //ADA
	$(document).on('keydown', '.video-play-pause.play, .video-play-pause.pause', function(e){
		if(e.keyCode === 13){
			e.preventDefault();
			$(this).trigger("click");
		}
