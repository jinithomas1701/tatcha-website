...
module.exports = function () {
    ...
    var flkty =  Flickity('.product-carousel',{ "cellAlign": "left","wrapAround": true, "contain": true, "imagesLoaded": "true", "percentPosition": false, "freeScroll": false, "pageDots": true, "groupCells": false, "lazyLoad": 1});
    ...
    if($(".video-icon-thumbnail").length > 0){
        $(".product-carousel .flickity-page-dots .dot:last-child").html($(".video-icon-thumbnail")[0])
    }
    ...
    $(document).on("click",".video-icon-thumbnail",function(){
        ...
        isVideoIconClick = false; // Prevent Autoplay in mobile
        switchProductThumbnail();
    })

    $('[data-pdp="videoFrameOverlay"]').on("click", function() {
        var videoSource = $('.product-thumbnail.video-thumbnail img').attr('data-source');
        if(player){
            if(videoSource == 'null' || videoSource === 'youtube'){
                playVideo();
...
function switchProductThumbnail(imageElement){
    ...
    if(typeof(imageElement) != 'undefined'){
        ...
        }else if($('.product-carousel .carousel-cell.is-selected').length > 0){
    ...
    if(videoSource == 'null' || videoSource === 'youtube'){
        ...
        if($(imageElement).attr("data-video") !== videoURL || isVariation) {
            ...
            player.on('pause', function() {
                $('[data-pdp="videoFrame"]').removeClass('video-cell--active');
            });
            player.on('play', function() {
                $('[data-pdp="videoFrame"]').addClass('video-cell--active');
            });
...
function onYoutubePlayerStateChange(event) {
    var $elem = $('[data-pdp="videoFrame"]');
    if (event.data === 0 || event.data === 2) {
        $elem.removeClass('video-cell--active');
    } else if (event.data === 1) {
        $elem.addClass('video-cell--active');
    }
}
