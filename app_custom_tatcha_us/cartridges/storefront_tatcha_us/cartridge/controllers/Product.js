    ...
    if (!showProductPageHelperResult.product.online && productType !== 'set' && productType !== 'bundle') {
        ...
        if (pageLookupResult.page) {
            ...
            res.render(showProductPageHelperResult.template, {
                Product: showProductPageHelperResult.fullProduct,
                ...
                DefaultVariant: showProductPageHelperResult.DefaultVariant,
                giftBuilderSKU : showProductPageHelperResult.giftBuilderSKU,
                maxOrderQuantity: showProductPageHelperResult.maxOrderQuantity,
                hasCartCategory: showProductPageHelperResult.hasCartCategory,
                mParticleProductJson: showProductPageHelperResult.mParticleProductJson,
                SorEnabled: showProductPageHelperResult.SorEnabled,
                lincWidgetProdData: showProductPageHelperResult.lincWidgetProdData,
                productPageView: showProductPageHelperResult.productPageView,
                afterpayEnable: showProductPageHelperResult.afterpayEnable,
                VideoThumbnailUrl: showProductPageHelperResult.VideoThumbnailUrl,
                YotpoReviewResponse: showProductPageHelperResult.YotpoReviewResponse,
                autoDeliveryCheck: showProductPageHelperResult.autoDeliveryCheck,
                oosStatus: showProductPageHelperResult.oosStatus,
                datas:showProductPageHelperResult.datas,
                VariationUtils:showProductPageHelperResult.VariationUtils,
                suggestedRoutine:showProductPageHelperResult.suggestedRoutine
            });
        }
    }
    next();
...
server.get('HitPPageCarousel', function (req, res, next) {
    var ProductFactory = require('*/cartridge/scripts/factories/product');
    var params = req.querystring;
    var productData = ProductFactory.get(params);
    var ProductMgr = require('dw/catalog/ProductMgr');
    var product = ProductMgr.getProduct(productData.id);
    var tileTemplate = 'product/components/productv1/carouseltileppage';
    var isVisible = ProductUtils.isVisible(product);

    if (isVisible) {
        res.render(tileTemplate, {
            product : product,
            Product : product
        });
        next();
...
server.get('ProductTile', function (req, res, next) {
    var ProductMgr = require('dw/catalog/ProductMgr');
    var params = req.querystring;
    var pid = params.pid;
    var parentpid = params.parentpid;
    var page = params.page;
    if(!empty(pid)){
        try {
            var product = ProductMgr.getProduct(pid);
            var parentProduct = ProductMgr.getProduct(parentpid);
            ...
            if(!empty(product) && ProductUtils.isVisible(product)) {
                    res.render(product.getTemplate() || template, {
                        product: product,
