...
// utility
var murmurhash = require('int_avatax_sfra/cartridge/scripts/murmurhash');
/**
...
avataxHelper.prototype = {
    ...
    getDefaultProductTaxCode: function () {
        return settingsObject.defaultProductTaxCode;
    },
...
function updateTaxes(basket, svcResponse) {
    try {
        ...
        for (count = 0; count < len; count++) {
            ...
            if (lineItem != null) {
                ...
                if (lineItem instanceof dw.order.ProductLineItem) {
                    ...
                    //RDMP-1827 : Fix for giftcard+product checkout from cart
                    lineItem.updateTax(0);
                } else {
        ...
        /**
         * RDMP-1827 : Fix for giftcard+product checkout from minicart
         */
        for each (var shipment in basket.shipments) {
            if (empty(shipment.shippingAddress)) {
                for each (let lineItem in shipment.shippingLineItems) {
                    lineItem.updateTax(0);
                }
            }
        }
...
function calculateTax(basket, orderNo) {
    ...
    //custom code
    if (session.custom.NoCall && session.custom.NoCall === true) {
        return;
    }

    if(basket.getAllProductLineItems().length == 0) {
        return {OK: true};
    }
    //custom code end
    ...
    try {
        ...
        while (pliIterator.hasNext()) {
            ...
            if (!empty(li.shipment.shippingAddress)) {
                ...
                line.taxCode = (!empty(li.getProduct()) && !empty(li.getProduct().taxClassID)) ? li.getProduct().taxClassID.toString() : defaultProductTaxCode;
                ...
                if (!empty(li.shippingLineItem)) {
                    ...
                    line.addresses.shipFrom = shipFromAddress;
                    line.addresses.shipTo = shipToAddress;
                    line.taxCode = defaultShippingMethodTaxCode;
                    line.customerUsageType = null;
        ...
        transactionModel.calculatedTax = basket.totalTax;
        ...
        if (session.privacy.avataxtransactionmodel && session.privacy.avataxtransactionmodel === hash && session.privacy.taxAmount && session.privacy.taxAmount === transactionModel.calculatedTax) {
        ...
        session.privacy.taxAmount=basket.totalTax;
