...
function execute(parameters) { // eslint-disable-line require-jsdoc
    ...
    function isPaymentInstrumentExistsInWallet(wallet, paymentMethodToken) {
        ...
        while (instrumentsIterator.hasNext()) {
            ...
            if (!empty(pi.creditCardToken) && pi.creditCardToken === paymentMethodToken) {
                exists = true;
                break;
            }
        }
    ...
    */
    ...
    function createCustomerPaymentInstrument(tokenizedMethodDetailsData, ignoreExistTokens) {
        ...
        try {
            ...
            Transaction.wrap(function () {
                newPaymentInstrument = customerWallet.createPaymentInstrument(PaymentInstrument.METHOD_CREDIT_CARD);
            });
            ...
            Transaction.wrap(function () {
                ...
                newPaymentInstrument.creditCardToken = paymentMethodToken;
            });
        } catch (err) {
            throw err;
        }
    ...
    function removeAllInstruments(file) {
        var importReader = new IO.FileReader(file);
        var importFileCsvReader = new IO.CSVStreamReader(importReader, ',', '"', 1);
        var line = '';
    ...
    function importTokensJob(opts) {
        var filePath = [IO.File.IMPEX, 'src', 'braintree', opts.fileShortName].join(IO.File.SEPARATOR);
        var importFile = new IO.File(filePath);
        var importReader = new IO.FileReader(importFile);
        var importFileCsvReader = new IO.CSVStreamReader(importReader, ',', '"', 1);
        var line = '';
