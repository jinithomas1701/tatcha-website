...
function getPreferredShippingAddress(req) {
    ...
    if(!empty(customerdPPAddress)) {
        var shippingAddressJson = JSON.parse(customerdPPAddress);
        shippingAddressJson.phone = (shippingAddressJson.phone)?shippingAddressJson.phone : "0000000000";
        customerdPPAddress = JSON.stringify(shippingAddressJson);
    }
...
function updateBillingForm(data) {
    ...
    data.viewData.phone = {
        value: (billingAddressToUpdate.phone) ? billingAddressToUpdate.phone :'0000000000'
    };
    data.viewData.email = {
 ...
 */
base.processForm = function processForm(req, paymentForm, viewFormData) {
