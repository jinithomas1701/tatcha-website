...
function makeCreditCardPayment(event) {
    ...
    //update form's default statecode value
    var $form = $('.billing-payment-sec');
    var country = $form.find('#billingCountry').val();
    if (country === 'US') {
        $form.find('#billingStatedefault').val($form.find('#billingStateUS').val());
    } else if (country === 'CA') {
        $form.find('#billingStatedefault').val($form.find('#billingStateNonUS').val());
    } else {
        var countryWithState = $form.find('#countryWithState').val();
        if (countryWithState.indexOf(country) > -1) {
            $form.find('#billingStatedefault').val($form.find('#billingStateText').val());
        } else {
