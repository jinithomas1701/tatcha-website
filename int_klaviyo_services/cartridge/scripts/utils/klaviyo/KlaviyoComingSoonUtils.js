...
function addUsertoComingSoonList(email, productId) {
	
	var additionStatus = false;
	
	var logger = Logger.getLogger('Klaviyo', 'KlaviyoComingSoonUtils - addUsertoComingSoonList()');
	
	var payload = createPayload(email, productId);
	var result = KlaviyoComingSoonService.call(payload);
	...
	if(!empty(resultObj)) {
		...
		if (!empty(response)) {
			if(response.status == 1) {
				...
				logger.error('addUsertoComingSoonList() failed for ' + email + '. Error from Klaviyo. Response: ' + response.response);
			}
		} else {
			logger.error('addUsertoComingSoonList() failed for ' + email + '. Error from Klaviyo. Response: ' + resultObj);
		}
	}
...
module.exports = {
	addUsertoComingSoonList : addUsertoComingSoonList
}

var KlaviyoComingSoonService = ServiceRegistry.createService('KlaviyoComingSoonService', {
	/**
