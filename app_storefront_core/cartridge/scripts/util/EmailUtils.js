...
function sendOrderEmail(order, mailType) {
	...
	try {
		...
		require('int_klaviyo_services/cartridge/scripts/utils/klaviyo/KlaviyoUtils').sendEmail(orderEmail, orderPayload, mailType);		
...
function sendOrderNotificationEmail(productList, mailType) {
	...
	try {
		...
		require('int_klaviyo_services/cartridge/scripts/utils/klaviyo/KlaviyoUtils').sendEmail(productList.owner.profile.email, orderPayload, mailType);
	} catch (e) {
...
function sendGiftCertificateEmail(giftCertificateLineItem, orderNo) {
	...
	try {
		...
		require('int_klaviyo_services/cartridge/scripts/utils/klaviyo/KlaviyoUtils').sendEmail(giftCertificateLineItem.recipientEmail, giftCardDetails, 'Received Gift Card');
...
function prepareGiftCertificateEmailPayload(giftCertificate, orderNo) {
	...
	giftCardDetails['RECIPIENT_URL'] = URLUtils.https('Search-Show', 'cgid', giftCertCategory , 'view', giftCertificate.merchantID, 'pmin', '0', 'pmax', giftCertificate.amount.value,'sz','149','is_gc', true).toString();
	...
	if(empty(giftCertificate.custom.giftCertificateImage)) {
		...
		var giftCardProductDetail = require('app_storefront_controllers/cartridge/scripts/app').getModel('Product').get(giftCardId).object;
...
function prepareOrderPayload(order, isFutureOrder, mailType) {
	...
	if(order.shipments.length > 0){
		...
		for(var j in productLineItems) {
		    ...
		    prdUrl = require('dw/web/URLUtils').url('Product-Show', 'pid', productLineItem.productID).toString();
		    var secondaryName = '';
		    // Get the product secondary name 
		    var productDetail = require('app_storefront_controllers/cartridge/scripts/app').getModel('Product').get(productLineItem.productID).object;
	...
	orderDetails['ITEM_CATEGORIES'] = itemCategories;
...
function prepareNotificationPayload(productList, isFutureOrder, mailType) {
	...
	for(var i=0; i<productList.productItems.length;i++) {
		...
		var productDetail = require('app_storefront_controllers/cartridge/scripts/app').getModel('Product').get(plItem.productID).object;
...
function sendThankyouEmail(giftCertificate, message) {
	...
	try {
		...
		require('int_klaviyo_services/cartridge/scripts/utils/klaviyo/KlaviyoUtils').sendEmail(giftCertificate.custom.giftCertificateSenderEmail, giftCardDetails, 'Gift Card Thank You Note');
